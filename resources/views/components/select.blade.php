...
@endif
<select data-default="{{ $model->$attr ?? 0 }}" {{ $multiple ? 'multiple' : '' }} name="{{ $id ? "ing[{$model->id}][$attr]" : $attr }}{{ $multiple ? '[]' : '' }}" class="custom-select rounded-0" id="{{ $id ? $attr.'-'.$model->id : $attr }}" @if((isset($model) && $model->isReadOnlyAttribute($attr)) || $disabled) disabled @endif>
    @if($nullable)
        <option
